 ...
 */
export async function lireTout(uid, optionTri) {
  const dossiers = [];
  /************************************************************** Exercice #5 : question A **************************/
  
  const infoTri = optionTri.split("-");
  console.log(optionTri);
  // Modifier très légèrement la ligne suivante
  const reponse = await firestore.collection(utilRef).doc(uid).collection(dossRef).orderBy(infoTri[0], infoTri[1]).get();
  reponse.forEach(
    doc => {
